...
```bash
# Run http server
$ docker run --rm -p 8080:80 ghcr.io/chinaran/go-httpbin:1.4-alpine3.17
# Run https server
$ docker run --rm -e HTTPS_CERT_FILE='/tmp/server.crt' -e HTTPS_KEY_FILE='/tmp/server.key' -p 8080:80 -v /tmp:/tmp ghcr.io/chinaran/go-httpbin:1.4-alpine3.17
```
